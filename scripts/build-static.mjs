...
const urls = [];
for (const lang of ['zh', 'en', 'ja']) for (const page of ['home', 'products', 'applications', 'about', 'contact', 'brand']) {
  if (lang === 'ja' && page === 'brand') continue;
  ...
  const html = template.replace('lang="zh-CN"', `lang="${lang === 'zh' ? 'zh-CN' : lang}"`).replace('<!--metadata-->', head + links).replace('data-page="home" data-lang="zh"', `data-page="${page}" data-lang="${lang}"`).replace('<!--app-->', render(page, lang));
...
console.log('Exported 17 pages in Chinese, English and Japanese to out/.');
